...
public class SoundSettings extends SettingsPreferenceFragment implements Indexable {
    ...
    private PreferenceCategory volumes;
	...
	volumes = (PreferenceCategory) findPreference(KEY_VOLUMES);
 	final PreferenceCategory sounds = (PreferenceCategory) findPreference(KEY_SOUND);
        ...
        if (mVoiceCapable) {
            mVolumeLinkNotification = (SwitchPreference) volumes.findPreference(KEY_VOLUME_LINK_NOTIFICATION);
            mRingPreference =
    ...
    private void initVolumeLinkNotification() {
        if (mVoiceCapable) {
            ...
            updateRingPreference();
            if (!linkEnabled) {
                volumes.addPreference(mNotificationPreference);
            } else {
                volumes.removePreference(mNotificationPreference);
            }
